public class Student implements Comparable<Student> {
     ...
     *
     * @param other this one doesnt get used coz the first one is used
     */
    ...
    //implemented comparable method that takes Student
    public int compareTo(Student other){
        int diff = this.lastName.compareTo(other.lastName);
        if(diff ==0)
            diff = this.firstName.compareTo(other.firstName);
        if(diff ==0)
            diff = this.age - other.age;
        if(diff ==0)
            diff = this.advisor.compareTo(other.advisor);
        return diff;
        //return -1;
    }
